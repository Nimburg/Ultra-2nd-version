...
tuple<long long int, long long int > SQL_2Comd_2Res(string Comd1, string Comd2, string server_name, string DBname, string user_name, string user_psw)
{
	...
	// Command 1
	long long int SQLreturn1_int = 0;
	if (SQL_SUCCESS != SQLExecDirect(sqlstatementhandle, (SQLCHAR*)Comd1.c_str(), SQL_NTS)) {
		...
		if (SQLFetch(sqlstatementhandle) == SQL_SUCCESS) {
			SQLGetData(sqlstatementhandle, 1, SQL_C_SLONG, &SQLreturn1_int, 0, NULL);
		}//End of while
	}
	// Command 2
	...
	long long int SQLreturn2_int = 0;
	if (SQL_SUCCESS != SQLExecDirect(sqlstatementhandle, (SQLCHAR*)Comd2.c_str(), SQL_NTS)) {
		...
		if (SQLFetch(sqlstatementhandle) == SQL_SUCCESS) {
			SQLGetData(sqlstatementhandle, 1, SQL_C_SLONG, &SQLreturn2_int, 0, NULL);
		}//End of while
	}
	...
	return  make_tuple(SQLreturn1_int, SQLreturn2_int);
}